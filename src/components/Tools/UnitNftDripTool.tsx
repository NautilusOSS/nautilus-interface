...
const MAX_SELECT = 6;
const INDEXER_BASE = `${MIMIR_API}/nft-indexer/v1/tokens`;
const PAGE_SIZE = 12;
const ONE_WEEK_SECONDS = 7 * 24 * 3600;

/** Only this wallet sees Approve (Dorks / Dorks V2 drip tools). */
const APPROVE_BUTTON_ALLOWED_ADDRESS =
...
export interface UnitNftDripToolConfig {
  nftContractId: number;
  dripContractId: number;
  rewardTokenContractId: number;
  rewardTokenDecimals: number;
  /** Accrual rate in smallest token units per week (must match on-chain math). */
...
function computeClaimableAmount(
  ...
  const amountDivisible =
    Math.floor(accrued / dripPerWeekRaw) * dripPerWeekRaw;
...
async function getNftDripInfoFromContract(
  ...
  const tryDrip = async (readonly: boolean): Promise<NftDripInfo | null> => {
    ...
    const result = await CONTRACT_CI.drip(
      Number(nftContractId),
      Number(tokenId)
    );
    if (!result.success) {
      console.warn("[UnitNftDripTool] drip() returned success=false", {
        nftContractId,
        tokenId,
        result,
      });
    ...
    if (rv === undefined || rv === null) {
      console.warn("[UnitNftDripTool] missing returnValue", result);
      return null;
    }
  ...
  try {
    ...
    console.error("[UnitNftDripTool] getNftDripInfoFromContract error:", e);
    return null;
  }
}
...
function formatRewardAmount(raw: number, decimals: number): string {
...
export interface UnitNftDripToolProps {
  config: UnitNftDripToolConfig;
}

export const UnitNftDripTool: React.FC<UnitNftDripToolProps> = ({ config }) => {
  ...
  const [dripInfoByKey, setDripInfoByKey] = useState<
    Record<string, NftDripInfo | null>
  >({});
  const [dripLoadingByKey, setDripLoadingByKey] = useState<
    Record<string, boolean>
  >({});
  const [dripRefreshTrigger, setDripRefreshTrigger] = useState(0);
  const [selectedKeys, setSelectedKeys] = useState<Set<string>>(new Set());

  const { rewardTokenDecimals: dec } = config;
  const sym = config.rewardSymbol;
  ...
  const handleClaimSelected = async () => {
    ...
    try {
      ...
      for (const t of withClaimable) {
        const txnO = (
          await builder.drip.claim(Number(t.contractId), Number(t.tokenId))
        ).obj;
        buildN.push({
          ...txnO,
          payment: 1e6 - 7000 + i++,
          note: new TextEncoder().encode(
            `claim ${sym} from ${t.contractId}-${t.tokenId}`
          ),
        });
      }
      ...
      toast.success(
        `Claimed ${formatRewardAmount(totalClaimed, dec)} ${sym} from ${withClaimable.length} NFT(s)`
      );
      setSelectedKeys((prev) => {
        const next = new Set(prev);
        withClaimable.forEach((t) =>
          next.delete(tokenKey(t.contractId, t.tokenId))
        );
        return next;
      });
  ...
  const handleClaimOne = async (t: ParsedToken) => {
    ...
    try {
      ...
      const toSign = (r.txns as string[]).map((b) =>
      ...
      await algosdk.waitForConfirmation(algodClient, txId, 4);
      toast.success(
        `Claimed ${formatRewardAmount(info.claimableAmount, dec)} ${sym}`
      );
      setDripRefreshTrigger((r) => r + 1);
    } catch (err) {
  ...
  const handleApprove = async () => {
    ...
    try {
      const { algodClient, indexerClient } = getAlgorandClients();
      const dripContractAddress = algosdk.getApplicationAddress(
        config.dripContractId
      );
      ...
      if (!balanceR.success) {
        throw new Error(`Failed to get ${sym} balance`);
      }
      const balance = balanceR.returnValue;
      if (balance <= 0) {
        toast.error(`You don't have any ${sym} to approve`);
        return;
      }
      const ci = new CONTRACT(
        config.rewardTokenContractId,
      ...
      const approveR = await ci.arc200_approve(dripContractAddress, balance);
      ...
      const { txId } = await algodClient
        ...
        .do();
      await algosdk.waitForConfirmation(algodClient, txId, 4);
      toast.success(`Approved ${sym} for drip`);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    const url = `${INDEXER_BASE}?contractId=${config.nftContractId}&owner=${activeAccount.address}`;
    fetch(url)
      .then((res) => res.json())
        ...
        const parsed: ParsedToken[] = list
          ...
          .map((t) => {
            ...
            return {
              ...
              name: metadata.name ?? `${config.collectionLabel} #${t.tokenId}`,
  ...
  }, [activeAccount?.address, config.nftContractId, config.collectionLabel]);

  useEffect(() => {
    if (!activeAccount?.address || tokens.length === 0) return;
    const totalPagesInner = Math.max(1, Math.ceil(tokens.length / PAGE_SIZE));
    const safePageInner = Math.min(Math.max(1, page), totalPagesInner);
    const start = (safePageInner - 1) * PAGE_SIZE;
    const end = safePageInner * PAGE_SIZE;
    const tokensOnPage = tokens.slice(start, end);
    let cancelled = false;
    tokensOnPage.forEach((t) => {
      const key = tokenKey(t.contractId, t.tokenId);
      setDripLoadingByKey((prev) => ({ ...prev, [key]: true }));
      getNftDripInfoFromContract(config, t.contractId, t.tokenId, activeAccount.address)
        .then((info) => {
          if (!cancelled) {
  ...
  }, [activeAccount?.address, tokens, page, dripRefreshTrigger, config]);
    ...
    <Container>
      <Card>
        <Hero>
          ...
          <CollectionLink href={`/#/collection/${config.nftContractId}`}>
            View Collection
          </CollectionLink>
          {" · "}
          <CollectionLink
            href={`https://voiager.xyz/token/${config.rewardTokenContractId}`}
            target="_blank"
        ...
        {!activeAccount?.address && (
          <EmptyState>
            ...
            Connect your wallet to see your {config.collectionLabel} NFTs and
            their {sym} drip rewards.
          </EmptyState>
        )}
        ...
        {activeAccount?.address && !loading && !error && tokens.length === 0 && (
          <EmptyState>
            <EmptyStateIcon>{config.emptyStateEmoji}</EmptyStateIcon>
                  ...
                  {selectedKeys.size > 0 && (
                    <>
                      <PaginationLabel>
                        ...
                        {formatRewardAmount(selectedClaimableAmount, dec)} {sym}
                      </PaginationLabel>
                      <ClaimButton
                        type="button"
                      ...
                      >
                        {isClaiming
                          ? "Claiming…"
                          : `Claim Selected (${selectedKeys.size})`}
                      </ClaimButton>
                        ...
                        {dripLoading ? (
                          ...
                          <DripInfoRow>
                            {dripInfo.claimableAmount > 0 ? (
                              <ClaimableAmount>
                                {formatRewardAmount(dripInfo.claimableAmount, dec)}{" "}
                                {sym}
                              </ClaimableAmount>
                            ) : (
                              <span>
        ...
        {activeAccount?.address === APPROVE_BUTTON_ALLOWED_ADDRESS && (
          <ApproveButton
            type="button"
            onClick={handleApprove}
            disabled={isApproving}
          >
            {isApproving ? "Approving..." : `Approve ${sym} for Drip`}
          </ApproveButton>
        )}
      </Card>
    </Container>
