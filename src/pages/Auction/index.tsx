...
const AuctionDetail: React.FC = () => {
  ...
  const canPlaceBid = () => {
    ...
    return (
      auction.active === 1 &&
      auction.endTimestamp > now &&
      auction.seller !== activeAccount.address
    );
  };
  ...
  const getMinimumBid = () => {
    if (!auction) return 0;
    const currentPrice = highestOffer;
    const minimumIncrement = 0.1 * 1e6; // 0.1 VOI minimum increment
    return (currentPrice + minimumIncrement) / 1e6;
  };
  ...
  const handlePlaceBid = async () => {
    ...
    try {
      ...
      toast.success(`Bid of ${bidAmount} VOI placed successfully!`);
        ...
        <Grid item xs={12} md={6}>
          <StyledCard $isDark={isDarkTheme}>
            <CardContent>
              ...
              {/* Show auction status for non-biddable auctions */}
              {!canPlaceBid() && auction && (
                  ...
                  <Box sx={{ textAlign: "center" }}>
                    {isAuctionEnded() ? (
                      <StyledChip
                        ...
                        size="medium"
                      />
                    ) : activeAccount?.address === auction.seller ? (
                      <StyledChip
                        $isDark={isDarkTheme}
                        label="You are the seller"
                        color="warning"
                        size="medium"
                      />
                    ) : !activeAccount ? (
          ...
          {/* Buy VOI Section */}
          <BuyVoiContainer $isDark={isDarkTheme}>
            <Typography
              variant="h6"
              sx={{
                color: isDarkTheme ? "#fff" : "#000",
            ...
            >
              Purchase VOI tokens directly to participate in auctions and
              marketplace activities.
            </Typography>

            <Box sx={{ display: "flex", justifyContent: "center", mb: 2 }}>
              <iframe
                src={`https://www.ibuyvoi.com/widget?destination=${
            ...
            >
              Powered by iBuyVOI • Secure payment processing
            </Typography>
          </BuyVoiContainer>{" "}
        </Grid>
